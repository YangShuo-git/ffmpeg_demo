    ...
    if (get_master_sync_type(is) != AV_SYNC_VIDEO_MASTER) {
        ...
        if (!isnan(diff) && fabs(diff) < is->max_frame_duration) {
            if (diff <= -sync_threshold)
                delay = FFMAX(0, delay + diff);    //上一帧持续时间往小的方向调整
            else if (diff >= sync_threshold && delay > AV_SYNC_FRAMEDUP_THRESHOLD)
                delay = delay + diff;              //上一帧持续时间往大的方向调整
...
static double vp_duration(VideoState *is, Frame *vp, Frame *nextvp) {
    ...
    { 
        ...
        if (isnan(duration)    // duration 数值异常
            ...
            return vp->duration; // 异常情况使用之前入队列的时候计算的帧间隔（根据帧率计算）。异常时以帧时间为基准(1秒/帧率)
        else
            return duration; //正常情况使用两帧pts差值计算duration
    } else {
        return 0.0;   // 不同播放序列, 序列不连续则返回0
    }
}
...
retry:
        if (frame_queue_nb_remaining(&is->pictq) == 0) { // 帧队列是否为空
            ...
            last_duration = vp_duration(is, lastvp, vp);  // 计算上一帧应显示的时长
            delay = compute_target_delay(last_duration, is);  // 计算上一帧还需要播放多久
...
//      of the seek_pos/seek_rel variables
            ...
            if (ret < 0) {
                ...
                if (is->subtitle_stream >= 0) {  // 如果有字幕流 同上
                    packet_queue_flush(&is->subtitleq);
                    packet_queue_put(&is->subtitleq, &flush_pkt);
                }
                if (is->video_stream >= 0) {    // 如果有视频流 同上
